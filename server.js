...
const PYTHON_CMD = process.env.PYTHON_CMD || path.join(__dirname, ".venv", "bin", "python3");
...
def load_model(model_path):
    ...
    if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
        ...
        classes = []
        config = {}
        raise ValueError("O checkpoint não contém 'model_state_dict'.")
        
    arch = config.get("arch", "unknown")
    if arch != "resnet18":
        raise ValueError(f"Incompatibilidade de modelo: esperado 'resnet18', encontrado '{arch}'.")
...
`;
/**
 * As linham abaxio salvam o script Python contido na variável PYTHON_WORKER_CODE em um arquivo físico no disco.
 * Mas por que isso é necessário?
 * O Node.js executa processos externos através da API 'child_process' (neste caso, spawn).
 * O binário do Python espera receber o caminho para um arquivo de script para executá-lo.
 * Como nosso código Python está embutido em uma string (String.raw) dentro do JavaScript, 
 * precisamos materializá-lo em um arquivo real (inference_worker.py) no diretório .runtime 
 * para que possamos iniciar o worker Python passando esse arquivo como argumento... não faça isso no seu código real, use o modelo real! :)
 */
